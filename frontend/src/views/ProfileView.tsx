...
import { faArrowRight, faBicycle, faPersonBiking } from "@fortawesome/free-solid-svg-icons";
...
export function ProfileView() {
  ...
  useEffect(() => {
    ...
    getRides().then(rides => setRides(rides.slice(0, 5)))
  }, [])
  ...
  <Heading marginTop={'10'} marginBottom={'10'}>Your rides:</Heading>
  <Accordion width={'100%'} marginBottom={'80px'}>
  {
    rides.map(ride => {
      return <AccordionItem key={ride.id}>
      <AccordionButton>
      <Box flex='1' textAlign='left' height='60px' sx={{
        display: 'flex',
        alignItems: 'center'
      }}>
      {
        Math.random() > 0.5 ? 
        <FontAwesomeIcon size='2x' icon={faPersonBiking} /> : 
        <FontAwesomeIcon size='2x' icon={faBicycle} />
      }
      <FontAwesomeIcon size='2x' icon={faArrowRight} style={{
        marginLeft: '10px'
      }}/>
      </Box>
      <h2 style={{
